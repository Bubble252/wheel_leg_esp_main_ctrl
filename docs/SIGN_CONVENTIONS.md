...
> 说明参考代码的多项式拟合系数已经吸收了负号 (即存储的是 -K_matlab)。
...
| φ/pitch 增大 | 前倾 *(见注)* | 后仰 (Pitch>0) | 后仰 (pitch>0) | ⚠️ 见注 |
| dφ/pitch_rate 增大 | 前倾加速 | 后仰加速 | 后仰加速 | ⚠️ 见注 |
| T > 0 | 驱动前进 | 驱动前进 | 驱动前进 | ✅ 一致 |
| Tp > 0 | 腿向后摆 | 腿向后摆 | 腿向后摆 | ✅ 一致 |

> **注 (φ/pitch 方向)**: MATLAB 中 φ 是从水平面量的角，前倾为正。但参考代码中 Pitch 是标准航空约定(后仰为正)。
> 这不影响控制，因为 K 矩阵拟合时已经吸收了这个符号差异。
> 实际上在 MATLAB 线性化后，K[4] (phi→T) 的符号使得: 前倾时 T > 0 (加速前进去追)，这与参考代码中 K[4]*(Pitch-offset) 的行为一致 (Pitch<0 时前倾, K[4]*(负值) 产生负 T… 但 K[4] 本身就是正的,所以 K[4]*负pitch = 负T = 减速? 不对...)
>
> **更精确的分析**: MATLAB 的 φ>0 = 前倾; 参考代码 Pitch>0 = 后仰。所以 φ_matlab = -Pitch_ref。
> K_matlab[4] 作用于 φ, K_ref[4] 作用于 Pitch。如果 K_ref = -K_matlab (因为 `u = -Kx`)，
> 则 `K_ref[4] * Pitch = (-K_matlab[4]) * (-φ) = K_matlab[4] * φ`，符号正确!
> 在多项式拟合中，这些符号变换已被吸收。
...
### 5.3 K 矩阵数值示例 (L0 = 0.07m)

| K 索引 | 状态变量 | 控制输出 | 典型值 | 物理含义 |
|--------|---------|---------|--------|---------|
| K[0] | θ → T | wheel_T | ≈ -3.83 | θ增大(后摆) → T减小(减速/后退) |
| K[1] | dθ → T | wheel_T | ≈ -0.38 | 后摆加速 → 减速 |
| K[2] | x → T | wheel_T | ≈ +0.71 | 前进过多 → 加速前进?? (存疑) |
| K[3] | v → T | wheel_T | ≈ +0.98 | 速度过快 → 继续加速?? (阻尼) |
| K[4] | φ → T | wheel_T | ≈ +3.56 | 后仰 → 前进 (追回重心) |
| K[5] | dφ → T | wheel_T | ≈ +0.26 | 后仰加速 → 前进 |
| K[6] | θ → Tp | leg_Tp | ≈ -15.3 | θ增大(后摆) → Tp<0(前摆回正) |
| K[7] | dθ → Tp | leg_Tp | ≈ -1.06 | 后摆加速 → 前摆阻尼 |
| K[8] | x → Tp | leg_Tp | ≈ +1.03 | |
| K[9] | v → Tp | leg_Tp | ≈ +0.44 | |
| K[10] | φ → Tp | leg_Tp | ≈ +17.1 | 后仰 → 腿前摆? (耦合复杂) |
| K[11] | dφ → Tp | leg_Tp | ≈ +0.76 | |

> **注**: 以上数值由多项式拟合在 L0=0.07m 处计算。实际值取决于物理参数。
...
- [ ] MATLAB φ (前倾正) vs 代码 Pitch (后仰正) 的符号变换在多项式拟合中是否完全吸收
